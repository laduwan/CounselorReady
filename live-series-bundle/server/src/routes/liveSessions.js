...
router.post('/:id/register', protect, async (req, res) => {
  try {
    ...
    (async () => {
      try {
        const registrant = await User.findById(req.user._id)
          .select('email profile.firstName profile.lastName');
        const name = `${registrant?.profile?.firstName || ''} ${registrant?.profile?.lastName || ''}`.trim()
          || registrant?.email || '';
        const dateStr = session.scheduledStart
        ...
        await sendAdminAlert('live_session_registered', {
          userName: name,
        ...
        console.error('[live] admin notification error:', notifyErr.message);
      }
    })();
  } catch (err) {
