...
### Route Registration
- **All routes are registered in `server/src/index.js`.** Do NOT add, remove, or rewrite any `import` or `app.use()` line without including ALL existing routes. The file header says "37 route mounts" — count them before and after your edit. If the count drops, you broke something.
- **Never rewrite index.js from scratch.** Only add new lines. The startup integrity check will catch regressions.
- Route file `interactiveCourseRoutes.js` (1109+ lines) is the FULL course pipeline. `courseRoutes.js` (665 lines) is a STRIPPED version missing certificate/evaluation/attestation/gamification. The import MUST point to `interactiveCourseRoutes.js`.
...
- **Never use emoji in buttons or UI elements.** They render as garbled characters or giant empty boxes.
...
- **NEVER use:** `#34495E` (old Navy), `#40634A` (old Forest), `#34503D`

### Logo Rules
- "CounselorReady" = two separate spans: "Counselor" + "Ready"
- On burgundy backgrounds: Counselor = `#f2a8be` (rose), Ready = `#98c3a9` (sage)
- On light backgrounds: Counselor = `#6B1D34` (burgundy), Ready = `#4A7C59` (hunter)
- CR monogram: C = rose/burgundy, R = green, on dark burgundy rounded square
...
- `!` character breaks `node -e` — use scripts instead

### Collections
- `interactivecourses` = active (all courses live here)
- `courses` = legacy (do not wire frontend to this)
- `usercourseprogresses` = progress tracking (lowercase pluralized)

### File Delivery
- Output to `/mnt/user-data/outputs/` then `present_files`
- For HTML edits, targeted `str_replace` on specific functions is more reliable than full rewrites
- Always validate JS with `new Function(code)` before delivering HTML with embedded scripts
