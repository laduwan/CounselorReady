...
(function () {
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="relative">
          <button onclick="toggleCRDropdown('notif')" class="p-2 rounded-lg hover:bg-stone-100 transition-colors relative">
            <svg class="w-5 h-5" style="color:#78716c" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
  ...
  // ── FOOTER HTML ───────────────────────────────────────────
  const policyLinksHTML = policyLinks.map(l =>
    `<a href="${l.href}" class="text-burgundy-400 hover:text-white text-sm transition-colors">${l.label}</a>`
  ).join('\n          ');

  const footerHTML = `
  <footer class="bg-burgundy-950 mt-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <div class="flex flex-col md:flex-row items-center justify-center gap-6 mb-6">
        <img src="/images/nbcc-provider-badge.jpg" alt="NBCC Approved Continuing Education Provider" class="h-20 w-auto">
        <div class="text-center md:text-left">
          <p class="text-white font-semibold text-sm">NBCC Approved Provider</p>
          <p class="text-burgundy-300 text-xs">ACEP #7760</p>
        </div>
      </div>
      <div class="mb-6">
        <p class="text-burgundy-400 text-xs leading-relaxed text-center max-w-4xl mx-auto">
      ...
      <div class="border-t border-burgundy-800 pt-6 flex flex-col md:flex-row justify-between items-center gap-4">
        <p class="text-burgundy-400 text-sm">&copy; ${new Date().getFullYear()} CounselorReady. All rights reserved.</p>
