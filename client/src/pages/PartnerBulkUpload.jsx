...
export default function PartnerBulkUpload() {
  ...
  function downloadTemplate() {
    const blob = new Blob([JSON.stringify({ courses: TEMPLATE }, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'course-upload-template.json';
    a.click();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Import multiple courses at once via JSON</p>
        </div>
        <button
        ...
        {mode === 'file' ? (
          <div>
            <label className="flex flex-col items-center justify-center border-2 border-dashed border-stone-300 rounded-xl p-8 cursor-pointer hover:border-stone-400 transition-colors">
              ...
              <p className="text-sm text-stone-600 font-medium">{file ? file.name : 'Click to select JSON file'}</p>
      ...
      {/* Format Guide */}
      <div className="card p-5">
        <h2 className="text-sm font-semibold text-stone-900 mb-2">JSON Format Guide</h2>
        <div className="text-xs text-stone-500 space-y-2">
          <p>Each course object requires: <code className="bg-stone-100 px-1 rounded">title</code>, <code className="bg-stone-100 px-1 rounded">description</code>, <code className="bg-stone-100 px-1 rounded">ceHours</code></p>
          <p>Optional fields: <code className="bg-stone-100 px-1 rounded">slug</code> (auto-generated from title), <code className="bg-stone-100 px-1 rounded">categories</code>, <code className="bg-stone-100 px-1 rounded">tags</code>, <code className="bg-stone-100 px-1 rounded">objectives</code>, <code className="bg-stone-100 px-1 rounded">presenter</code>, <code className="bg-stone-100 px-1 rounded">status</code> (draft/published)</p>
        </div>
      </div>
    </div>
  );
