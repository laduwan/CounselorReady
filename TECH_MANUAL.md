...
### 2026-09-01 — Deep-cloning a course with JSON stringifies `sections[]._id`, orphaning learner progress
Section subdocuments in `interactivecourses` carry real `ObjectId` `_id` values, and `UserCourseProgress.sectionId` refers to them. Any patch script that builds its working copy with `JSON.parse(JSON.stringify(doc))` — a natural reflex when mutating a raw-driver read — silently converts those to plain strings, so the `$set` writes back string ids and every enrolled learner's section progress stops matching. Caught while converting CR-307's inline knowledge checks; the fix used there is to take a second independent read as the working copy and assert the section ids are still ObjectIds both pre-write and on read-back. Any script that mutates `sections[]` should carry the same guard.

### 2026-09-15 — `POST /api/admin/broadcast` recorded `sendEmail` but never sent anything
The Admin Hub broadcast modal's "Send email" checkbox reached `adminCourses.js`, which saved `sendEmail: true` on the Announcement and stopped there — only `POST /api/announcements` (`announcements.js`, its module-local `sendAnnouncementEmails`) actually calls Resend, and nothing imports it across files. The admin route now sends its own copy. Related constraint: there is no job anywhere that sweeps `{ sendEmail: true, emailSent: false }`, so a future-dated broadcast can set its banner window via `startDate` but its email cannot be scheduled — the route sends only when `startDate <= now` and reports `emailStatus: 'deferred'` otherwise. A sweep in `dailyNotificationCheck.js` would close that gap, but that file is on the protected list and needs Ke's call.
