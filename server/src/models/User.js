...
userSchema.methods.hasUsedQuarterlyConsult = function() {
  if (!this.consultations || !Array.isArray(this.consultations)) return false;
...
userSchema.methods.canBookConsultation = function() {
  ...
  const status = this.subscription?.status || 'inactive';
  
  // Must be active VIP
  if (!['vip', 'annual_vip', 'lifetime'].includes(plan)) {
    return { allowed: false, reason: 'VIP subscription required' };
  }
  
  if (status !== 'active' && status !== 'lifetime' && status !== 'trial') {
