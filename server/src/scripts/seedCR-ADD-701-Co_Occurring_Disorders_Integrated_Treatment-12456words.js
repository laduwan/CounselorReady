/**
 * seedCR-ADD-701-Co_Occurring_Disorders_Integrated_Treatment-12456words.js
 * CounselorReady · GAITP LLC · NBCC ACEP #7760
 *
 * Replaces the retired strict-false script whose filename claimed 19,500 words —
 * canonical count of the authored content is 12,456 (verified via
 * utils/courseWordCount.js). Block shapes corrected to canonical
 * (scenarioTree node fields normalized). 3 CE target is 18,000: course ships
 * DRAFT and stays draft until content is expanded or CE hours are revised.
 *
 * Wrapper: _seedTemplate.js pattern — model upsert via doc.save() (pre-save
 * hook computes wordCount/totalContentBlocks) + DB read-back verification.
 *
    ...
    },
    {
      ...
      "contentBlocks": [
        ...
        },
        {
          "type": "fillInBlank",
          "order": 8,
          "title": "In the IDDT model, the four stages of integrated treatment are: ________, persuasion, active treatment, and ________.",
          "question": "In the IDDT model, the four stages of integrated treatment are: ________, persuasion, active treatment, and ________.",
          "blanks": [
            { "prompt": "First blank", "answer": "engagement" },
            { "prompt": "Second blank", "answer": "relapse prevention" }
          ],
...
async function seed() {
  ...
  // DB read-back — verify the write landed with hook-computed fields
  ...
  console.log(`✅ DB verified ${saved.courseCode} — wordCount=${saved.wordCount}, totalContentBlocks=${saved.totalContentBlocks} (target ${(saved.ceHours || 0) * 6000})`);
  if (saved.wordCount < (saved.ceHours || 0) * 6000) {
    console.warn('⚠ UNDER word target — remains draft by design. Expand content or revise CE hours before publishing.');
  }
  await mongoose.disconnect();
}
