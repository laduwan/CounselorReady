...
router.get('/consult-status', protect, async (req, res) => {
  try {
    // Ensure user methods exist and handle gracefully
    let canBook = { allowed: false, reason: 'Unable to determine' };
    let currentQuarter = '';
    let nextQuarter = '';
    let usedThisQuarter = false;
    
    if (typeof req.user.canBookConsultation === 'function') {
      canBook = req.user.canBookConsultation();
    }
    if (typeof req.user.getCurrentQuarter === 'function') {
      currentQuarter = req.user.getCurrentQuarter();
    }
    if (typeof req.user.getNextQuarter === 'function') {
      nextQuarter = req.user.getNextQuarter();
    }
    if (typeof req.user.hasUsedQuarterlyConsult === 'function') {
      usedThisQuarter = req.user.hasUsedQuarterlyConsult();
    }
