      ...
      case 'checkout.session.completed': {
        ...
        if (session.metadata?.type === 'live-session') {
          ...
          if (live) {
            ...
            (async () => {
              try {
                const buyer = await User.findById(liveUserId).select('email profile.firstName profile.lastName');
                const amountDollars = session.amount_total ? (session.amount_total / 100).toFixed(2) : '0';
                const dateStr = live.scheduledStart
                  ? new Date(live.scheduledStart).toLocaleString('en-US', {
                      timeZone: 'America/New_York', dateStyle: 'short', timeStyle: 'short'
                    }) + ' ET'
                  : 'TBD';
                if (twilioClient && process.env.ADMIN_PHONE) {
                  twilioClient.messages.create({
                    body: `CounselorReady: Live Session Purchase\n${buyer?.email || ''} paid $${amountDollars} for "${live.title}"\n${dateStr}`,
                    from: process.env.TWILIO_PHONE_NUMBER,
                    to: process.env.ADMIN_PHONE
                  }).catch(e => logger.error({ err: e, userId: liveUserId, requestId: req.requestId }, 'SMS live session purchase notification error'));
                }
              } catch (notifyErr) {
                logger.error({ err: notifyErr, userId: liveUserId, requestId: req.requestId }, 'Live session purchase admin notification error');
              }
            })();
          }
          break;
