...
export async function generateCertificate(data) {
  ...
  try {
    const acepLogoBytes = fs.readFileSync(ACEP_LOGO_PATH);
    const acepLogoImage = await pdfDoc.embedJpg(acepLogoBytes);
    const acepLogoDims = acepLogoImage.scale(0.4); // Scale to appropriate size
    
    // Position in bottom left area
    page.drawImage(acepLogoImage, {
      x: 55,
      y: 38,
      width: 50,
      height: 50
    ...
    console.log('ACEP logo not found:', e.message);
  }
  ...
  // ACEP statement - adjusted position to account for logo
  ...
  page.drawText(acepStatement, {
    x: (width - acepStatementWidth) / 2 + 25,
    y: 50,
    size: 6,
    font: helvetica,
    color: COLORS.darkForest
  ...
  page.drawText(acepStatement2, {
    x: (width - acepStatement2Width) / 2 + 25,
    y: 40,
    size: 6,
    font: helvetica,
    color: COLORS.darkForest
